...
const faqs = [
  ...
  {
    q: 'Есть риск бана?',
    a: 'Встроена многоуровневая система маскировки — рандомизация сигнатур, мимикрия процессов, маскировка памяти. Однако любые модификации — это риск. Настоятельно рекомендуем использовать альтернативный аккаунт.',
  },
  {
  ...
  },
  {
    ...
    a: 'Код полностью открыт — можешь проверить каждую строку. Скрипт не собирает данные, не обращается к сторонним серверам, не требует интернет-подключения и лишних разрешений. Работает локально.',
  },
  {
    q: 'Как часто выходят обновления?',
    a: 'Оффсеты обновляются в течение 24-48 часов после каждого обновления игры. Крупные обновления функций — примерно раз в 2-3 недели. Обновление оффсетов можно запустить прямо из меню скрипта.',
  },
  {
    q: 'Скрипт не запускается — что делать?',
    a: 'Проверь: 1) версия GameGuardian 101.1 или выше, 2) выбран правильный процесс Hypper Sandbox, 3) игра полностью загружена, 4) root-доступ активен. Если не помогает — перезагрузи устройство и попробуй снова.',
  },
  {
  ...
  },
  {
    ...
    a: 'Частично. На BlueStacks и LDPlayer возможны проблемы с инъекцией. Лучшая совместимость — на реальном устройстве с Magisk. MEmu и Nox — тестируем, нет гарантий.',
  },
];
      ...
      <div className="max-w-3xl mx-auto px-6">
        <div className="mb-16">
          ...
          <p className="mt-4 text-[14px] text-white-30 max-w-lg">
            Ответы на самые популярные вопросы. Если не нашёл ответ — напиши в комментарии к скрипту.
          </p>
        </div>
        ...
        <div>
          {faqs.map((faq, i) => {
            ...
            return (
              <div
                key={i}
                className="border-b border-white-8 first:border-t"
              >
                <button
                ...
                >
                  ...
                  <span
                    className={`text-[20px] text-white-15 shrink-0 transition-all duration-300 ${
